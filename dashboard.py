...
# Utilisation de CSS personnalisé pour appliquer le zoom à la barre latérale
st.sidebar.markdown(
    f"""
    <style>
...
# Prédiction sur l'ensemble du jeu de données au lancement de l'application
predictions = model.predict_proba(df_complet.drop("SK_ID_CURR", axis=1))
df_complet["Prediction_Class_0"] = predictions[:, 0]  
df_complet["Prediction_Class_1"] = predictions[:, 1]

# Seuil pour la détermination de la classe
seuil = 0.2222

# Création de la colonne de classe en fonction du seuil
df_complet["Classe"] = (predictions[:, 1] > seuil).astype(int)
...
if selected_client:
    ...
    style = "font-size: 24px; font-family: Verdana, sans-serif; font-weight: bold; margin-left: -375px; margin-top:-525px"
    ...
    with col1:    
        ...
        emoji_style = "font-size: 48px; margin-left: -375px;"
    ...
    # Accessibilité des données du client sous la forme d'une selectbox présente dans la barre latérale
        style_1bis = "font-size: 24px; font-family: Verdana, sans-serif; font-weight: bold; margin-left: -375px; margin-top:0px"
    ...
    with col1:   
        # Donnée demandée
        style_1ter = "font-size: 20px; font-family: Verdana, sans-serif; font-weight: bold; margin-left: -375px; margin-top:0px"
        st.markdown(f"<p style='{style_1ter}'>Voici la donnée demandée</p>", unsafe_allow_html=True)
        style_1q = "font-size: 18px; font-family: Verdana, sans-serif; font-weight: bold; margin-left: -375px; margin-top:0px"
...
#######################################
    ...
    with col2:
        # Calcul des valeurs SHAP à partir du DataFrame original
        shap_df = df_complet[df_complet['SK_ID_CURR'] == selected_client].drop(["SK_ID_CURR", "Prediction_Class_0", "Prediction_Class_1", "Classe"], axis=1)
        explainer = shap.TreeExplainer(model)

        # Prédictions sur le nouveau client pris au hasard
        shap_df = client_data.drop(["SK_ID_CURR", "Prediction_Class_0", "Prediction_Class_1", "Classe"], axis=1)
        shap_values = explainer(shap_df, check_additivity=False)

        # Obtention de la prédiction pour le client
        prediction_shap = model.predict_proba(shap_df)[0]

        if prediction_shap[0] <= 0.2222:
        # Affichez les valeurs SHAP pour la classe 0
            fig_shap_bons_client= shap.plots._waterfall.waterfall_legacy(explainer.expected_value[0], shap_values[0].values[:,0], shap_df.iloc[0], max_display=11)
            st.pyplot(fig_shap_bons_client)
        else:
        # Affichez les valeurs SHAP pour la classe 1
            fig_shap_mauvais_client= shap.plots._waterfall.waterfall_legacy(explainer.expected_value[0], shap_values[0].values[:,0], shap_df.iloc[0], max_display=11)
            st.pyplot(fig_shap_mauvais_client)
...
with col3:
    ...
    probas = model.predict_proba(df_complet.drop(["SK_ID_CURR", "Prediction_Class_0", "Prediction_Class_1","Classe"], axis=1))
    ...
    if selected_client:
        client_index = df_complet[df_complet['SK_ID_CURR'] == selected_client].index[0]
...
###############################################
# DEUXIEME PARTIE: LES VALEURS DE SHAPLEY
# #############################################
...
@st.cache_data  # Mettez en cache cette fonction pour qu'elle ne soit pas recalculée à chaque rafraîchissement de la page
def calculate_shap_values(df_complet):
    # Calcul des valeurs SHAP globales à l'aide du TreeExplainer
    shap_values_global = explainer.shap_values(df_complet.drop(["SK_ID_CURR", "SK_ID_CURR", "Prediction_Class_0", "Prediction_Class_1","Classe"], axis=1))
...
mean_shap_values, sorted_indices = calculate_shap_values(df_complet)
...
ax.set_yticklabels(df_complet.drop(["SK_ID_CURR", "SK_ID_CURR", "Prediction_Class_0", "Prediction_Class_1","Classe"], axis=1).columns[sorted_indices], fontsize=40)
st.pyplot(fig)
...
if "Montant du crédit selon l'âge" in selected_graphiques:
    ...
    color_map = {'Accord': 'lightskyblue', 'Refus': 'salmon'}
    ...
    fig_age_credit.add_trace(go.Scattergl(x=df_complet[df_complet['Décision'] == 'Refus']['AGE'],
                                             ...
                                             mode='markers',
                                             marker=dict(size=5, color='salmon'),
                                             name="Refus"))
...
if "Montant du crédit selon le revenu" in selected_graphiques:
    ...
    # Personnalisation des titres et de la légende
    fig_income_credit.update_layout(title="MONTANT DE CREDIT SELON LE REVENU",
                                    xaxis_title="REVENU",
                                    yaxis_title="MONTANT DU CREDIT",
                                    legend_title_text="DECISION")
